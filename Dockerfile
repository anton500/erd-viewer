FROM alpine:3.15 AS graphviz_builder
RUN set -ex; \
    ...
    cd /usr/src; \
    wget -O graphviz.tar.gz "https://gitlab.com/api/v4/projects/4207231/packages/generic/graphviz-releases/3.0.0/graphviz-3.0.0.tar.gz"; \
...
FROM python:3.10-alpine3.15 AS uwsgi_builder
RUN set -ex; \
    apk add --no-cache --virtual build_deps \
        build-base \
        linux-headers \
