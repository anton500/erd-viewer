...
from erd_viewer.database import Database, Table, Schema, Column, ColumnWithPath
...
class DBJSONDecoder(json.JSONDecoder):
    ...
    def __object_decoder(self, obj: dict) -> Union[Schema, Table, Column, ColumnWithPath, dict]:
        ...
        elif set(ColumnWithPath.__annotations__.keys()).issubset(obj.keys()): # pylint: disable=no-member
            return ColumnWithPath(schema=obj['schema'], table=obj['table'], column=obj['column'])
        else:
            return obj
