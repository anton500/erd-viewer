...
class Graph:
    ...
    _HTML_TABLE_ROW_TEMPLATE = (
        '<tr><td port="{port}">{name}</td><td>{datatype}</td></tr>'
    )
    ...
    def _get_html_table(self, schema: str, table: str, columns: list[Column]) -> str:
        ...
        for column in columns:
            tbody += self._HTML_TABLE_ROW_TEMPLATE.format(
                port=column.name, name=column.name, datatype=column.type
            )
            ...
            for column in columns:
                if schema_table not in unvisited_tables:
                    for fk_ref in column.fk_references:
                        if (
                            SchemaTable(fk_ref.schema, fk_ref.table)
                            in tables_with_columns
                        ):
                            refs.append(
                                (
                                    SchemaTableColumn(
                    ...
                    for fk_ref in column.fk_references:
                        if SchemaTable(fk_ref.schema, fk_ref.table) in set(
                            tables_with_columns
                        ).difference(unvisited_tables):
                            refs.append(
                                (
                                    SchemaTableColumn(
                                        schema_table.schema,
    ...
    def _fill_digraph(
        ...
        digraph: Digraph,
        tables_with_columns: dict[SchemaTable, list[Column]],
        refs: list[tuple[SchemaTableColumn, SchemaTableColumn]],
    ) -> Digraph:

        for schema_table, columns in tables_with_columns.items():
            digraph.node(
                name=f"{schema_table.schema}.{schema_table.table}",
                label=self._get_html_table(
                    schema_table.schema, schema_table.table, columns
                ),
            )
    ...
    def build_digraph(
        ...
        tables_with_columns = {
            schema_table: self.get_columns(schema_table) for schema_table in tables
        }
        ...
        if onlyrefs:
            ...
            for schema_table, columns in tables_with_columns.items():
                ...
                for column in columns:
                    if (
                        SchemaTableColumn(
                            schema_table.schema, schema_table.table, column.name
                        )
                        in unique_columns
                    ):
...
class RelatedTables(Graph):
    def __init__(
        ...
        depth: int,
        tables_to_exclude: list[SchemaTable],
        onlyrefs: bool = False,
        graph_name: str = None,
        engine: str = "neato",
        graph_attr: dict = None,
        ...
        self.tables, self.unvisited_tables = self._get_related_tables(
            {schema_table}, depth, tables_to_exclude
        )
    ...
    def _get_related_tables(
        self,
        unvisited: set[SchemaTable],
        depth: int,
        tables_to_exclude: list[SchemaTable],
        visited: set[SchemaTable] = None,
    ) -> tuple[set, set]:
        if visited is None:
            visited = set()
        ...
        for schema_table in unvisited.copy():
            ...
            for column in self.get_columns(schema_table):
                for ref in column.fk_references + column.pk_references:
                    ...
                    if (
                        ref_schema_table not in visited
                        and ref_schema_table not in tables_to_exclude
                    ):
                        unvisited.add(ref_schema_table)

        return self._get_related_tables(
            unvisited, depth - 1, tables_to_exclude, visited
        )
